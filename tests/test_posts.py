...
@pytest.mark.parametrize("title,content,published",[
    ("new title","best content",True),
    ("new pizza","pepperoni",True),
    ("heyho","ulala",True)
])
...
def test_create_post_default_published_true(authorized_client,test_posts,test_user):
    res = authorized_client.post("/posts/",json={"title":"albala","content":"fsafasf"})
    created_post = schemas.Post(**res.json())
    assert created_post.title == "albala"
    assert created_post.content == "fsafasf"
    assert created_post.published is True
    assert created_post.owner_id == test_user['id']
    assert res.status_code == 201

def test_unauthorized_user_create_post(client,test_user,test_posts):
    res = client.post("/posts",json={"title":"safafa","content":"sfafsafa"})
    assert res.status_code == 401
...
def test_update_post_non_existent(authorized_client,test_user,test_posts):
    ...
    res = authorized_client.put(f"/posts/8388",json=data)
    assert res.status_code == 404